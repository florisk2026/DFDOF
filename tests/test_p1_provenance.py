...
""",
    ...
    assert all(
        item.to_dict()["stored_path"] is not None for item in state.input_evidence
    )
    assert all(
        cast(Path, item.stored_path).suffix.lower()
        in {ext.lower() for ext in SUPPORTED_IMAGE_EXTENSIONS}
        for item in state.input_evidence
    )
...
def test_run_phase_1_multi_type_listing_uses_priority(
...
""",
    ...
    assert record["identified_as"] in (
        "controller_ios",
        "controller_android",
        "drone_sd",
    )
...
def test_run_phase_1_uses_single_mmls_attempt_for_e01(
    tmp_path: Path, monkeypatch
) -> None:
...
""",
        ...
        if exe.startswith("fls"):
            assert "-o" in command
            return subprocess.CompletedProcess(
                command, 0, stdout="r/r 1234: DCIM/100MEDIA/DJI_0001.MP4\n"
            )
        raise AssertionError(f"unexpected command: {command}")
    ...
    assert (
        sum(
            ...
            if Path(command[0]).name.lower().startswith("mmls")
        )
        == 1
    )
    assert all(
...
def test_run_phase_1_reports_clear_mmls_failure(tmp_path: Path, monkeypatch) -> None:
    ...
    def fake_run(command, capture_output=True, text=True, check=False, stdout=None):
        ...
        if exe.startswith("mmls"):
            return subprocess.CompletedProcess(
                command, 1, stdout="", stderr="unsupported image type"
            )
...
def test_run_phase_1_fallbacks_to_fls_without_mmls(tmp_path: Path, monkeypatch) -> None:
    ...
    assert record["operator_confirmed"] is True
    assert any(
        flag.startswith("p1_mmls_unavailable_fls_direct:")
...
def test_prompt_blocks_continuation_for_unidentified_sources(
    monkeypatch, capsys
) -> None:
