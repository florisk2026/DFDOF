...
def test_run_phase_3_android_logical(tmp_path: Path, monkeypatch) -> None:
	...
	android_evidence = Evidence(android_zip, acquisition_method="logical", type="input", skip_hash=True)
	...
	assert all(item["artefact_category"] != DATABASES or "other.db" not in item["stored_path"] for item in artefacts)
...
def test_run_phase_3_ios_parsed_files(tmp_path: Path, monkeypatch) -> None:
	...
	ios_parsed_root = documents_dir / "dfdof_output" / "CASE-P3-2" / "p2_image_parsing" / "controller_ios_parsed"
	ios_parsed_root.mkdir(parents=True, exist_ok=True)
	flight_record = ios_parsed_root / "domains" / "AppDomain-com.dji.go" / "Documents" / "FlightRecords" / "record1.txt"
	flight_record.parent.mkdir(parents=True, exist_ok=True)
	flight_record.write_text("flight record", encoding="utf-8")
	account_plist = ios_parsed_root / "domains" / "AppDomain-com.dji.go" / "Library" / "Preferences" / "com.dji.go.plist"
	...
	ios_evidence = Evidence(ios_zip, acquisition_method="logical", type="input", skip_hash=True)
...
def test_run_phase_3_drone_flight_storage_flat_dat(tmp_path: Path, monkeypatch) -> None:
	...
	flight_evidence = Evidence(flight_zip, acquisition_method="logical", type="input", skip_hash=True)
	...
	assert not any(flag.startswith("p3_dji_export_not_recognised") for flag in result.anomaly_flags)



def test_run_phase_3_android_physical_filters_extensions(tmp_path: Path, monkeypatch) -> None:
	project_root = tmp_path
	documents_dir = project_root / "Documents"
	documents_dir.mkdir(parents=True)
	monkeypatch.setattr(Path, "home", lambda: project_root)
	...
	android_evidence = Evidence(android_image, acquisition_method="physical", type="input", skip_hash=True)
	state.input_evidence.append(android_evidence)
	state.phase_outputs["p1_provenance"]["identified_evidence"] = [
		{
	...
	def fake_extract_tsk_image(*_args, **kwargs):
		...
		if category == DATABASES:
			...
			evidence_items.extend([
				Evidence(good_db.name, stored_path=good_db, parent=android_evidence, type="extracted", artefact_category=category),
				Evidence(bad_db.name, stored_path=bad_db, parent=android_evidence, type="extracted", artefact_category=category),
			])
			...
			evidence_items.extend([
				Evidence(good_dat.name, stored_path=good_dat, parent=android_evidence, type="extracted", artefact_category=category),
				Evidence(bad_bin.name, stored_path=bad_bin, parent=android_evidence, type="extracted", artefact_category=category),
			])
...
def test_run_phase_3_drone_sd_physical_single_pass(tmp_path: Path, monkeypatch) -> None:
	...
	sd_evidence = Evidence(sd_image, acquisition_method="physical", type="input", skip_hash=True)
